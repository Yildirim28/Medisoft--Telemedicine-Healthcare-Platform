    ...
    <div>
      <FadeIn delay={0}>
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Seat Bookings</h1>
        </div>
      </FadeIn>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-gray-300">Check-in</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-gray-300">Check-out</th>
            ...
            <tbody>
              {bookings.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No seat bookings found</td></tr>
              )}
              {bookings.map(function (b, idx) {
                var statusColors = {
                  ...
                  'Checked-Out': 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400',
                ...
                return (
                  <tr key={b.booking_id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 animate-fadeInUp" style={{ animationDelay: (idx * 60) + 'ms', animationFillMode: 'forwards', opacity: 0 }}>
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-full text-xs font-semibold">{b.seat_type || '-'}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{b.hospital_name || '-'}</td>
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{formatDate(b.check_in_date)}</td>
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{formatDate(b.check_out_date)}</td>
                    <td className="px-4 py-3">
