...
export default function AdminArticles() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <FadeIn delay={100}>
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 mb-6 border dark:border-gray-700 animate-bounce-in">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-gray-300">Author</th>
            ...
            <tbody>
              {articles.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No articles found</td></tr>
              )}
              {articles.map(function (a, idx) {
                return (
                  <tr key={a.article_id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 animate-fadeInUp" style={{ animationDelay: (idx * 60) + 'ms', animationFillMode: 'forwards', opacity: 0 }}>
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{a.article_id}</td>
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{a.title}</td>
                    <td className="px-4 py-3">
                      {a.category ? (
                        <span className="px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-full text-xs font-semibold">{a.category}</span>
                      ) : '-'}
                    </td>
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{a.author_name || '-'}</td>
                    <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{a.created_at ? new Date(a.created_at).toLocaleDateString() : '-'}</td>
                    <td className="px-4 py-3">
