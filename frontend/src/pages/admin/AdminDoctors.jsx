...
export default function AdminDoctors() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6 shadow-sm animate-fade-in-up" style={{ animationDuration: '400ms', animationFillMode: 'forwards' }}>
          <h2 className="text-lg font-semibold mb-4">{editingDoctor ? 'Edit Doctor' : 'Add New Doctor'}</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              {form.specialization !== '__other__' ? (
                <select value={form.specialization} onChange={function(e) { setForm(Object.assign({}, form, { specialization: e.target.value })); }} required className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-white">
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="is_verified" checked={form.is_verified} onChange={function(e) { setForm(Object.assign({}, form, { is_verified: e.target.checked })); }} className="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500" />
              <label htmlFor="is_verified" className="text-sm font-medium text-gray-700">Verified Doctor</label>
            </div>
      ...
      <FadeIn delay={100}>
        <div className="mb-4">
          <input type="text" placeholder="Search doctors..." value={search} onChange={function(e) { setSearch(e.target.value); }} className="w-full md:w-80 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all duration-300 hover:border-gray-400" />
        </div>
      </FadeIn>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {doctors.length === 0 && (
                <tr><td colSpan="7" className="px-4 py-8 text-center text-gray-500 animate-fade-in">No doctors found</td></tr>
              )}
              {doctors.map(function(doc, idx) {
                return (
                  <tr key={doc.doctor_id || doc.id} className="hover:bg-gray-50 transition-all duration-200 active:scale-[0.99]" style={{ animation: 'fadeIn 0.3s ease-out forwards', animationDelay: (idx * 50) + 'ms', opacity: 0 }}>
                    ...
                    <td className="px-4 py-3">
                      {doc.is_verified ? (
                        ...
                        <span className="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-500 rounded-full text-xs font-medium">Unverified</span>
                      )}
                    </td>
