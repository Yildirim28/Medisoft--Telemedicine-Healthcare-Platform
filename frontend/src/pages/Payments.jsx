      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-center gap-3 py-3 px-4 bg-white rounded-xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-700">bKash Secured</span>
          </div>
        ...
        {showForm && (
          ...
          <form onSubmit={handleBkashPayment} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6 animate-bounce-in">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1 mb-3">Select the service you want to pay for — amount will auto-populate</p>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {SERVICE_TYPES.map((s) => (
                  ...
                  >
                    ...
                    <span className={`text-xs font-medium ${selectedService === s.value ? 'text-pink-700' : 'text-gray-600'}`}>{s.label}</span>
                    {s.defaultPrice > 0 && (
                      <span className="text-[10px] text-gray-400">৳{s.defaultPrice}</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            {selectedInfo && (
              <div className={`p-4 rounded-xl bg-gradient-to-r ${selectedInfo.color} bg-opacity-10 border border-gray-100`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{selectedInfo.label}</p>
                    <p className="text-xs text-gray-500">{selectedInfo.description}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    min="1"
                    placeholder="0.00"
                    className={`input-modern pl-8 ${selectedInfo?.defaultPrice > 0 ? 'bg-gray-50' : ''}`}
                    value={amount}
                ...
                {selectedInfo?.defaultPrice === 0 && (
                  <p className="text-xs text-gray-400 mt-1">Enter the amount to pay for this order</p>
                )}
              </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              <p className="text-xs font-semibold text-gray-600 mb-2">Accepted Payment Methods:</p>
              <div className="flex flex-wrap gap-2">
                {['bKash', 'Nagad', 'Rocket', 'Visa', 'Mastercard', 'DBBL', 'IBBL'].map((m) => (
                  <span key={m} className="px-3 py-1 bg-white rounded-full text-xs font-medium text-gray-600 border border-gray-200">
                    {m}
                  </span>
                ))}
              </div>
        ...
        {!showForm && (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div className="text-center max-w-md mx-auto">
              ...
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Secure Payments via bKash</h2>
