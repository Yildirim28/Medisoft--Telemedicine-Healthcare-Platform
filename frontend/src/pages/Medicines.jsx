          ...
          <div className="space-y-6">
              ...
              <form onSubmit={handleAddMedicine} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select className="input-modern bg-white" value={mForm.category} onChange={(e) => { const val = e.target.value; setMForm({ ...mForm, category: val }); if (val !== 'Other') setCustomCategory(''); }}>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    {medicines.map((m) => (
                      <div key={m.medicine_id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-gray-800 truncate">{m.brand_name || m.name || 'Medicine'}</h3>
                            {m.generic_name && <p className="text-xs text-gray-500 mt-0.5">{m.generic_name}</p>}
              ...
              <div className="w-full lg:w-[380px] flex-shrink-0">
                <div className="lg:sticky lg:top-4 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  ...
                  {cart.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-300 text-xs mt-1">Add medicines from the shop</p>
                    </div>
                  ) : (
                    <>
                      <div className="space-y-3 max-h-[320px] overflow-y-auto pr-1">
                        {cart.map((c) => (
                          <div key={c.medicine_id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <div className="min-w-0">
                                <p className="font-medium text-gray-800 text-sm truncate">{c.brand_name || c.name || 'Medicine'}</p>
                                <p className="text-xs text-gray-500">Qty: {c.quantity} × ৳{c.price}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                              <span className="font-semibold text-gray-800 text-sm">৳{(parseFloat(c.price || 0) * c.quantity).toFixed(2)}</span>
                              <button
                ...
                <div key={o.order_id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-all duration-300">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{new Date(o.placed_at || o.created_at).toLocaleString()}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-600">
                    <div className="flex items-center gap-2">
                      ...
                      Total: <span className="font-semibold text-gray-800">৳{o.total_price || o.total_amount}</span>
                    </div>
                    {o.delivery_address && (
                      <span className="text-gray-400 text-xs truncate max-w-[200px]" title={o.delivery_address}>📍 {o.delivery_address}</span>
                    )}
                  </div>
                  {canCancelOrder(o) && (
                  ...
                  {o.status === 'Placed' && !canCancelOrder(o) && (
                    <div className="mt-3 pt-3 border-t border-gray-100">
                      <span className="text-xs text-gray-400 italic">Cancellation window expired</span>
                    </div>
                  )}
                </div>
              );
